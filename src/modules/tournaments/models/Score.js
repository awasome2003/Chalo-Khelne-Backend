...
// Hot path (Phase 5): every live-score read/write hits Score by matchId. Score
// has no tournamentId, so the live-scoring index is keyed on matchId (+ clubId
// for tenant-scoped reads). Previously Score had NO indexes → collection scans
// on every score update.
ScoreSchema.index({ matchId: 1, clubId: 1 });
